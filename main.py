...
def reset_timer():
  ...
  #resets the timer_lable "Timer"
  timer_label.config(text="Timer", font=(FONT_NAME, 30))
  #reset checkmarks to no marks
...
def start_timer():
  ...
  global reps
  reps += 1
  #determins the lenght of each section
  ...
  if reps % 8 == 0:
    ...
    #othervise its the work time timer
    count_down(work_sec)
...
def count_down(count):
  """starts the timer from the given input int and updates the canvas timer_text to show the countdown on screen stops after it reaches 0"""
  #find the min and seconds(math.floor option flors the number to the closest one so 4.8 would still be 4 untill it becomes 5.0 etc)
  ...
  if count > 0:
    #if timer left is greater than 0 we execute an update on the window every second or 1000ms and we call the count_down function on itself so it would keep going.
    #count - 1 is the positional argunet we put in the function that is called and we update the count by reducing it by 1 that iw equal to 1s
    global timer
...
#Start button
start_but = Button(text="Start", font=(FONT_NAME, 10), highlightthickness=0, command=start_timer)
start_but.grid(column=0,row=2)

#reset button
reset_but = Button(text="Reset", font=(FONT_NAME, 10), highlightthickness=0, command=reset_timer)
