...
def start_timer():
    """calls the count-down function to start the timer when the Start button is pressed"""
    ...
    if reps % 8 == 0:
        # if its 8th rep then its long break,
        # we can tell if it's cleanly divided with no remainders
        count_down(long_break_sec)
        timer_label.config(text="Break", fg=RED)
    elif reps % 2 == 0:
        # if its 2/4/6 rep then its long break,
        # we can tell if it's cleanly divided by 2 since it would be even
        count_down(short_break_sec)
        timer_label.config(text="Break", fg=PINK)
    else:
        # otherwise its work time timer
        count_down(work_sec)
...
def count_down(count):
    ...
    if count > 0:
        # if the timer left is greater than 0, we execute an update on the window
